...
| Grok Build | root `plugin.json`, `.grok-plugin/marketplace.json`, `skills/`, `commands/` | `grok plugin install DietrichGebert/ponytail --trust`, then enable. Invoke the explicit-only skill with `/ponytail`. Grok lifecycle hooks are not used because passive hook output cannot inject instructions. |
